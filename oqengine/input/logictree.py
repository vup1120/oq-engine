...
class BranchSet(object):
            ...
            if key == 'applyToTectonicRegionType':
                ...
                if value == 'area':
                    if not isinstance(source,
                                      oqhazardlib.source.AreaSource):
                        return False
                elif value == 'point':
                    # area source extends point source
                    if (not isinstance(
                            source, oqhazardlib.source.PointSource) \
                            or isinstance(
                                    source,
                                    oqhazardlib.source.AreaSource)):
                        return False
                    ...
                    if not isinstance(
                        source, oqhazardlib.source.SimpleFaultSource):
                        return False
                elif value == 'complexFault':
                    if not isinstance(
                        source, oqhazardlib.source.ComplexFaultSource):
                        return False
                else:
    ...
    def apply_uncertainty(self, value, source):
        ...
        if not isinstance(source.mfd, oqhazardlib.mfd.TruncatedGRMFD):
