...
def get_running_job(job_id):
    ...
    (:class:`oqengine.db.models.OqJob`), load all of the calculation
    data from the database and KVS and return a
    :class:`oqengine.engine.JobContext` object.

    If the calculation is not currently running, a
    :exc:`JobCompletedError` is raised.

    :returns:
        :class:`oqengine.engine.JobContext` object, representing an
    ...
    :raises JobCompletedError:
        If :meth:`~oqengine.engine.JobContext.is_job_completed` returns
        ``True`` for ``job_id``.
    """
