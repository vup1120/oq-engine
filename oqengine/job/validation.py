...
AVAILABLE_GSIMS = oqhazardlib.gsim.get_available_gsims().keys()
...
class BaseOQModelForm(ModelForm):
    ...
    :class:`oqengine.db.models.Input`, keyed by config file parameter
...
def _validate_imt(imt):
    ...
    valid_imts = list(set(oqhazardlib.imt.__all__) - set(['SA']))
...
def gsim_is_valid(mdl):
    if mdl.gsim in AVAILABLE_GSIMS:
        return True, []
    return False, ['The gsim %r is not in in oqhazardlib.gsim' % (
            mdl.gsim)]
