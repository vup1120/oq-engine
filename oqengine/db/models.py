...
def profile4job(job_id):
    ...
    :returns: a :py:class:`oqengine.db.models.OqJobProfile` instance
    """
...
def inputs4job(job_id, input_type=None, path=None):
    ...
    :returns: a list of :py:class:`oqengine.db.models.Input` instances
    """
...
class HazardCalculation(djm.Model):
    ...
    # We we create a `oqhazardlib.site.SiteCollection` for the
    ...
    def site_collection(self):
        """
        Get the :class:`oqhazardlib.site.SiteCollection` for this
        calculation.
    ...
    def init_site_collection(self):
        ...
        :class:`oqhazardlib.site.SiteCollection` which represents
    ...
    def points_to_compute(self):
        """
        Generate a :class:`~oqhazardlib.geo.mesh.Mesh` of points.
...
def get_site_collection(hc):
    ...
    :returns:
        :class:`oqhazardlib.site.SiteCollection` instance.
    """
    ...
    if len(site_data) > 0:
        ...
        sites = zip(site_data.lons, site_data.lats, site_data.vs30s,
                    ...
                    site_data.z2pt5s)
        sites = [oqhazardlib.site.Site(
            oqhazardlib.geo.Point(lon, lat), vs30, vs30m, z1pt0, z2pt5)
        ...
        sites = [
            oqhazardlib.site.Site(pt, hc.reference_vs30_value,
                                          measured,
                            hc.reference_depth_to_2pt5km_per_sec,
                            hc.reference_depth_to_1pt0km_per_sec)
            for pt in points]

    return oqhazardlib.site.SiteCollection(sites)
...
class RiskCalculation(djm.Model):
    ...
    def output_container_builder(self, risk_calculator):
        """
        :returns: a dictionary mapping oqengine.db.models.Output ids
...
class ExposureModel(djm.Model):
    ...
    def get_asset_chunk(self, taxonomy, region_constraint, offset, count):
        """
        :returns: a list of `oqengine.db.models.ExposureData` objects
...
class AssetManager(djm.GeoManager):
    ...
    def contained_in(self, exposure_model_id, taxonomy,
        ...
        `oqengine.db.models.ExposureModel` with ID equal to
        `exposure_model_id`
        """
        return list(
            self.raw("""
