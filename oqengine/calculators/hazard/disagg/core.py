...
from oqengine.calculators.hazard.classical import core as classical
from oqengine.db import models
from oqengine.input import logictree
from oqengine.input import source
from oqengine.utils import general as general_utils
...
def disagg_task(job_id, block, lt_rlz_id, calc_type):
    ...
    :param block:
        ...
        :class:`oqhazardlib.site.Site` objects.

        For more info, see
        :func:`oqengine.calculators.hazard.classical.core.\
compute_hazard_curves`
        if ``calc_type`` is 'hazard_curve' and :func:`compute_disagg` if
        ``calc_type`` is 'disagg'.
    :param lt_rlz_id:
        ID of the :class:`oqengine.db.models.LtRealization` for this
...
def compute_disagg(job_id, sites, lt_rlz_id):
    ...
    8. Call the hazardlib calculator
       (see :func:`oqhazardlib.calc.disagg.disaggregation` for more
       info).

    :param int job_id:
        ID of the currently running :class:`oqengine.db.models.OqJob`
    :param list sites:
        `list` of :class:`oqhazardlib.site.Site` objects, which
        indicate the locations (and associated soil parameters) for which we
        need to compute disaggregation histograms.
    :param int lt_rlz_id:
        ID of the :class:`oqengine.db.models.LtRealization` for which
    ...
    tom = oqhazardlib.tom.PoissonTOM(hc.investigation_time)
    ...
    src_site_filter = oqhazardlib.calc.filters.\
        source_site_distance_filter(hc.maximum_distance)
    rup_site_filter = oqhazardlib.calc.filters.\
    ...
    for imt, imls in hc.intensity_measure_types_and_levels.iteritems():
        ...
        for site in sites:
            ...
            for poe in hc.poes_disagg:
                ...
                bin_edges, diss_matrix = oqhazardlib.calc.disagg.\
...
def _save_disagg_matrix(job, site, bin_edges, diss_matrix, lt_rlz,
    ...
    :class:`~oqengine.db.models.DisaggResult`).

    :param job:
        :class:`oqengine.db.models.OqJob` representing the current job.
    :param site:
        :class:`oqhazardlib.site.Site`, containing the location
        geometry for these results.
    :param bin_edges, diss_matrix
        The outputs of :func:`oqhazardlib.calc.disagg.disaggregation`.
    :param lt_rlz:
        :class:`oqengine.db.models.LtRealization` to which these
        results belong.
...
def _prepare_sources(hc, lt_rlz_id):
    ...
    :param int lt_rlz_id:
        ID of a :class:`oqengine.db.models.LtRealization`

    :returns:
        A generator of hazardlib source objects for the given realization of
        the given calculation. See :mod:`oqhazardlib.source` for more
...
class DisaggHazardCalculator(haz_general.BaseHazardCalculatorNext):
    ...
    def get_task_complete_callback(self, hc_task_arg_gen, block_size,
        ...
        See
        :meth:`oqengine.calculators.hazard.general.\
