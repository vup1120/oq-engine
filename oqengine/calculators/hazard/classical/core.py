...
from oqengine.calculators.post_processing import (
...
def compute_hazard_curves(job_id, src_ids, lt_rlz_id):
    ...
    if hc.maximum_distance:
        ...
        calc_kwargs['source_site_filter'] = (
            oqhazardlib.calc.filters.source_site_distance_filter(dist))
        calc_kwargs['rupture_site_filter'] = (
            oqhazardlib.calc.filters.rupture_site_distance_filter(
                dist))

    # mapping "imt" to 2d array of hazard curves: first dimension -- sites,
    # second -- IMLs
    logs.LOG.debug('> computing hazard matrices')
    matrices = oqhazardlib.calc.hazard_curve.hazard_curves_poissonian(
        **calc_kwargs)
