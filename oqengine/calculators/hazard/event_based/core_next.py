...
:mod:`oqhazardlib.calc.stochastic`.
...
:mod:`oqhazardlib.calc.gmf`.
"""

import random

import oqhazardlib.imt
import oqhazardlib.source
import numpy.random

from django.db import transaction
from oqhazardlib.calc import filters
from oqhazardlib.calc import gmf as gmf_calc
from oqhazardlib.calc import stochastic
...
from oqengine.calculators.hazard.event_based import post_processing
from oqengine.db import models
from oqengine.db.aggregate_result_writer import MeanCurveWriter
from oqengine.db.aggregate_result_writer import QuantileCurveWriter
from oqengine.input import logictree
from oqengine.job.validation import MAX_SINT_32
from oqengine.utils import stats
from oqengine.utils import tasks as utils_tasks
...
def ses_and_gmfs(job_id, src_ids, lt_rlz_id, task_seed, result_grp_ordinal):
    ...
    database. See :class:`oqengine.db.models.SESCollection`.
...
def _create_gmf_cache(n_sites, imts):
    ...
    :param imts:
        A `list` or other sequence of :mod:`oqhazardlib.imt` IMT
        objects.
    """
    cache = dict()
...
def _save_ses_rupture(ses, rupture, complete_logic_tree_ses,
    ...
    :param ses:
        A :class:`oqengine.db.models.SES` instance. This will be DB
        'container' for the new rupture record.
    :param rupture:
        A :class:`oqhazardlib.source.rupture.Rupture` instance.
    :param complete_logic_tree_ses:
        :class:`oqengine.db.models.SES` representing the `complete
    ...
    is_from_fault_source = rupture.source_typology in (
        oqhazardlib.source.ComplexFaultSource,
        oqhazardlib.source.SimpleFaultSource)
...
def _save_gmfs(gmf_set, gmf_dict, points_to_compute, result_grp_ordinal):
    ...
    :param gmf_set:
        A :class:`oqengine.db.models.GmfSet` instance, which will be
    ...
    :param points_to_compute:
        An :class:`oqhazardlib.geo.mesh.Mesh` object, representing all
    ...
    for imt, gmfs in gmf_dict.iteritems():
        ...
        sa_period = None
        sa_damping = None
        if isinstance(imt, oqhazardlib.imt.SA):
...
def _create_gmf_record(gmf_set, imt):
    """
    Helper function to create :class:`oqengine.db.models.Gmf` records.
    The record will be saved to the DB and returned.

    :param gmf_set:
        :class:`oqengine.db.models.GmfSet` instance.
    :param imt:
        An instance of one of the IMT classes defined in
        :mod:`oqhazardlib.imt`.

    :returns:
        The newly created :class:`oqengine.db.models.Gmf` object.
    """
    gmf = models.Gmf(
...
class EventBasedHazardCalculator(haz_general.BaseHazardCalculatorNext):
    ...
    def initialize_ses_db_records(self, lt_rlz):
        """
        Create :class:`~oqengine.db.models.Output`,
        :class:`~oqengine.db.models.SESCollection` and
        :class:`~oqengine.db.models.SES` "container" records for
    ...
    def _compute_investigation_time(haz_calc):
        ...
        :param haz_calc:
            :class:`oqengine.db.models.HazardCalculation` object for
            the current job.
        """
    ...
    def initialize_gmf_db_records(self, lt_rlz):
        """
        Create :class:`~oqengine.db.models.Output`,
        :class:`~oqengine.db.models.GmfCollection` and
        :class:`~oqengine.db.models.GmfSet` "container" records for
        a single realization.
