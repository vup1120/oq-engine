...
from oqhazardlib import correlation
...
from oqengine.input import source
from oqengine.job.validation import MAX_SINT_32
from oqengine.job.validation import MIN_SINT_32
...
def store_source_model(job_id, seed, params, calc):
    ...
    :type calc: :class:`oqengine.input.logictree.LogicTreeProcessor`
        instance
    """
...
def store_site_model(input_mdl, site_model_source):
    ...
    :returns:
        `list` of :class:`oqengine.db.models.SiteModel` objects. These
...
def validate_site_model(sm_nodes, mesh):
    ...
    :param sm_nodes:
        Sequence of :class:`~oqengine.db.models.SiteModel` objects.
    :param mesh:
        A :class:`oqhazardlib.geo.mesh.Mesh` which represents the
...
def get_site_model(hc_id):
    """Get the site model :class:`~oqengine.db.models.Input` record
    for the given job id.

    :param int hc_id:
        The id of a :class:`~oqengine.db.models.HazardCalculation`.

    :returns:
        The site model :class:`~oqengine.db.models.Input` record for
        this job.
    :raises:
...
def get_closest_site_model_data(input_model, point):
    ...
    site model :class:`~oqengine.db.models.Input` and
    :class:`oqhazardlib.geo.point.Point`.

    :param input_model:
        :class:`oqengine.db.models.Input` with `input_type` of
        'site_model'.
    :param site:
        :class:`oqhazardlib.geo.point.Point` instance.

    :returns:
        The closest :class:`oqengine.db.models.SiteModel` for the given
...
def store_site_data(hc_id, site_model_inp, mesh):
    ...
    :param int hc_id:
        ID of a :class:`~oqengine.db.models.HazardCalculation`.
    :param site_model_inp:
        An :class:`~oqengine.db.models.Input` with an
        `input_type` == 'site_model'. This tells us which site model dataset to
        query.
    :param mesh:
        Calculation points of interest, as a
        :class:`oqhazardlib.geo.mesh.Mesh`.
    :returns:
        The :class:`oqengine.db.models.SiteData` object that was
...
def gen_sources(src_ids, apply_uncertainties, rupture_mesh_spacing,
    ...
    :param src_ids:
        A list of IDs for :class:`oqengine.db.models.ParsedSource`
        records.
    :param apply_uncertainties:
        A function to be called on each generated source. See
        :meth:`oqengine.input.logictree.LogicTreeProcessor.\
parse_source_model_logictree_path`

    For information about the other parameters, see
    :func:`oqengine.input.source.nrml_to_hazardlib`.
    """
...
def im_dict_to_hazardlib(im_dict):
    ...
    :returns:
        ...
        :mod:`oqhazardlib.imt` for more information.
    """
...
def imt_to_hazardlib(imt):
    ...
    :param str imt:
        ...
        equivlent hazardlib object. See :mod:`oqhazardlib.imt`.
    """
    if 'SA' in imt:
        match = re.match(r'^SA\(([^)]+?)\)$', imt)
        period = float(match.group(1))
        return oqhazardlib.imt.SA(period, models.DEFAULT_SA_DAMPING)
    else:
        imt_class = getattr(oqhazardlib.imt, imt)
        return imt_class()
...
def update_realization(lt_rlz_id, num_items):
    ...
    :param int lt_rlz_id:
        ID of the :class:`oqengine.db.models.LtRealization` we want
        to update.
    :param int num_items:
...
def get_correl_model(hc):
    ...
    :param hc:
        A :class:`oqengine.db.models.HazardCalculation` instance.

    :returns:
        A correlation object. See :mod:`oqhazardlib.correlation` for
        more info.
    """
...
class BaseHazardCalculatorNext(base.CalculatorNext):
    ...
    def computation_mesh(self):
        """
        :class:`oqhazardlib.geo.mesh.Mesh` representing the points
    ...
    def hc(self):
        ...
        :class:`~oqengine.db.models.HazardCalculation`.
        """
    ...
    def initialize_sources(self):
        ...
        :class:`~oqengine.db.models.Input` records for all of them,
        parse then, and save the parsed sources to the `parsed_source` table
        (see :class:`oqengine.db.models.ParsedSource`).
        """
    ...
    def initialize_site_model(self):
        ...
        :func:`oqengine.calculators.hazard.general.\
validate_site_model`.)
    ...
    def initialize_realizations(self, rlz_callbacks=None):
        ...
        :param rlz_callbacks:
            ...
            A :class:`~oqengine.db.models.LtRealization` object.
        """
    ...
    def initialize_source_progress(lt_rlz, hzrd_src):
        ...
        :param lt_rlz:
            :class:`oqengine.db.models.LtRealization` object to
            initialize source progress for.
        :param hztd_src:
            :class:`oqengine.db.models.Input` object that needed parsed
