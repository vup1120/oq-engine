...
from oqengine.calculators import base, post_processing
from oqengine import export
from oqengine.utils import stats
from oqengine.calculators.risk import hazard_getters
from nrmllib.risk import parsers

# FIXME: why is a writer in a package called "input" ?
from oqengine.input import exposure as exposure_writer
...
class BaseRiskCalculator(base.CalculatorNext):
    ...
    def hazard_outputs(self, hazard_calculation):
        """
        :returns: a list of :class:`oqengine.db.models.Output`
    ...
    def hazard_output(self, output):
        ...
        :class:`oqengine.db.models.HazardCurve'

        :param hazard_output: the ID of an
        :class:`oqengine.db.models.Output` object
    ...
    def parse_vulnerability_model(self, retrofitted=False):
        ...
        `:class:oqrisklib.scientific.VulnerabilityFunction` instance.
        """
        ...
        for record in parsers.VulnerabilityModelParser(path):
            if self.imt is None:
                self.imt = record['IMT']
            vfs[record['ID']] = oqrisklib.scientific.\
...
def write_loss_curve(loss_curve_id, asset, asset_output):
    """
    Stores and returns a :class:`oqengine.db.models.LossCurveData`
    where the data are got by `asset_output` and the
    :class:`oqengine.db.models.LossCurve` output container is
    identified by `loss_curve_id`.

    :param int loss_curve_id: the ID of the output container
    :param asset: an instance of
        :class:`oqengine.db.models.ExposureData`
    :param asset_output: an instance of
    :class:`oqrisklib.models.output.ClassicalOutput` or of
    :class:`oqrisklib.models.output.ProbabilisticEventBasedOutput`
    returned by risklib
    """
...
def write_loss_map_data(id, asset_ref, value, std_dev, location):
    """
    Create :class:`oqengine.db.models.LossMapData`
...
def write_loss_map(loss_map_ids, asset, asset_output):
    """
    Create :class:`oqengine.db.models.LossMapData` objects where the
    data are got by `asset_output` and the
    :class:`oqengine.db.models.LossMap` output containers are got by
    `loss_map_ids`.

    :param dict loss_map_ids: A dictionary storing that links poe to
    :class:`oqengine.db.models.LossMap` output container
...
def write_bcr_distribution(bcr_distribution_id, asset, asset_output):
    """
    Create a new :class:`oqengine.db.models.BCRDistributionData` from
    `asset_output` and links it to the output container identified by
    `bcr_distribution_id`.

    :param int bcr_distribution_id: the ID of
    :class:`oqengine.db.models.BCRDistribution` instance that holds
    the BCR map
    ...
    :param asset_output: an instance of
        :class:`oqrisklib.models.output.BCROutput` that holds BCR
