...
from oqengine.calculators import hazard
from oqengine.calculators.hazard.general import store_gmpe_map
from oqengine.calculators.hazard.general import store_source_model
from oqengine.db import models
from oqengine.engine import JobContext
from oqengine import engine
from oqengine import engine2
from oqengine import logs
from oqengine.input.logictree import LogicTreeProcessor
...
def store_hazard_logic_trees(a_job):
    ...
    :param a_job:
        :class:`oqengine.engine.JobContext` instance.
    """
...
class DbTestCase(object):
    ...
    def setup_job_profile(cls, job, force_inputs, save2db=True):
        ...
        :param job: The :class:`oqengine.db.models.OqJob`
            instance to use.
        ...
        :returns: a :class:`oqengine.db.models.OqJobProfile` instance
        """
...
def get_hazard_job(cfg, username=None):
    ...
    :class:`oqengine.db.models.OqJob` object for a hazard calculation.
    """
...
def get_risk_job(risk_demo, hazard_demo, output_type="curve", username=None):
    ...
    calculation and then creates a :class:`oqengine.db.models.OqJob`
