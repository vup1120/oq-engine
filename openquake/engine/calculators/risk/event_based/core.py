...
from openquake.engine.utils import tasks, general, config
...
def do_event_based(unit, containers, params, monitor):
    ...
    stats = []
    for rupture_id in outputs:
        stats.append(
            unit.workflow.statistics(
                [out.output for out in outputs[rupture_id]],
                unit.getter.weights(),
                params.quantiles,
                post_processing))
...
class EventBasedRiskCalculator(base.RiskCalculator):
    ...
    def task_arg_gen(self):
        ...
        ses_collections = set(ho.gmf.lt_realization.lt_model.ses_collection
                              for ho in gmf_outputs)

        for ses_coll in ses_collections:
            all_ruptures = models.SESRupture.objects.filter(
                rupture__ses_collection=ses_coll)

            blocks = general.SequenceSplitter(nblocks).split(all_ruptures)

            for taxonomy, site_assets in self.taxonomy_site_assets.iteritems():
                for ruptures in blocks:

                    calculation_units = []
                    for loss_type in models.loss_types(self.risk_models):
                        calculation_units.extend(
                            self.calculation_units(
                                loss_type, taxonomy, site_assets, ruptures))
    ...
    def calculation_units(self, loss_type, taxonomy, site_assets, ruptures):
        """
        :returns:
        ...
        time_span, tses = self.hazard_times()
        rlzs = set(ruptures[0].rupture.ses_collection.lt_model)
        hazard_outputs = [ho for ho in self.rc.hazard_outputs()
                          if ho.gmf.lt_realization in rlzs]
        sitecol = self.rc.get_hazard_calculation().site_collection
        for ho in hazard_outputs:
            yield workflows.CalculationUnit(
                ...
                hazard_getters.GroundMotionValuesGetter(
                    ho, site_assets, risk_model.imt,
                    ruptures, sitecol.sids))
