...
class GmfCollector(object):
    ...
    def __init__(self, params, imts, gsim_by_rlz):
        ...
        """
        self.imt_dict = models.Imt.get_imt_dict()
        self.params = params
        self.imts = imts
    ...
    def gen_gmf(self, r_sites, rupture, rupture_id, rupture_seed):
        """
        ...
        for rlz, gsim in self.gsim_by_rlz.items():
            ...
            for imt, gmf_1_realiz in gmf_dict.iteritems():
                ...
                yield rlz, imt, gmf_1_realiz.reshape(-1)

    def save_gmf(self, r_sites, rup, ses_rup):
        """
        """
        for rlz, imt, gmvs in self.gen_gmf(
                r_sites, rup, ses_rup.id, ses_rup.seed):
            gmf = models.Gmf.objects.get(lt_realization=rlz)
            models.GmfRupture.objects.create(
                rupture=ses_rup, gmf=gmf, imt=self.imt_dict[str(imt)],
